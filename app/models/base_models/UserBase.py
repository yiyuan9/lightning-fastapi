...
class UserBase(TableBase):
    ...
    username: str | None = Field(
        unique=True, index=True, description="用户名,系统内唯一"
    )
    is_active: bool = Field(default=True, description="账号是否激活")
    phone_number: str | None = Field(
        unique=True, default=None, index=True, description="手机号,可选但必须唯一"
    )
    hashed_password: str | None = Field(
        default=None, description="密码哈希值,不存储原始密码"
    )
    is_superuser: bool = Field(default=False, description="是否为超级管理员")
    shortid: str | None = Field(
        unique=True, index=True, default=None, description="用户短ID,系统内唯一"
    )
